...
int main ()
{
   // 1. array declaration- khai báo mảng
   int M[3]; // mảng nguyên có 3 phần tử
   int M2[7]; // mảng nguyên có 7 phần tử


   // 2. array initialization - khởi tạo mảng (có gán giá trị)
   int m[] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
   string m2[] = {"tong", "dang", "quang"};


   // 3. Retrieve the elements of an array - truy xuất các phần tử của mảng
   cout << m[0] << endl;
   cout << m[1] << endl;
   cout << m2[2] << endl;


   // 4. check size of an array - kiểm tra chiều dài của mảng
   cout << "chieu dai mang m[] = " << size (m) << endl;
   cout << "chieu dai mang m2[] = " << size (m2) << endl;


   // 5. browse array and print to the screen - duyệt mảng và in ra màn hình
   cout << "m[] = ";
   for (int pt : m)
   {
      cout << pt << " ";
...
// 7. change values for array - thay đôi giá trị cho mảng 
// cách 1: change direct - thay đổi trực tiếp
int main()
{
...
// cách 2: change vales by for loop - thay đôi giá trị bằng vòng lặp
...
// 9. sort array ascending - sắp xếp tăng dần
...
// 10. sort aray descending - sắp xếp giảm dần
// cách 1: dùng thư viện
void sort_array(int m[], int n)
{
   cout << "enter the number elements for array: "; cin >> n;
...
// 11. reverse array - đảo ngược mảng
void reverse_array (m[], int n)
{
   cout << "enter the number elements for array: "; cin >> n;
   reverse (m, m + n);
   cout << "reversed array: ";
...
// 12. create an array random interval - tạo một mảng ngẫu nhiên
void sort_array(int m[], int n)
{
   random_device rd;
   mt19937 gen(rd());
...
// 13. insert a value into index location - chèn phần tử vào mảng
...
// 17. find max element in the array
